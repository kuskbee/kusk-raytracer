...
public:
	...
	void Initialize(HWND window, int width, int height, int canvasWidth, int canvasHeight)
	{
		...
		if (canvasTexture)
		{
			...
			std::cout << "CreateTexture2D() - canvas error" << std::endl;
		}
		...
		// Create an index buffer
		{
			...
			bufferDesc.BindFlags = D3D11_BIND_INDEX_BUFFER;		// use as an index buffer
			bufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;		// allow CPU to write in buffer
	...
	void Update()
	{
		...
		if (i % 3 == 0)
			...
			v = Vec4{ 0.0f, 1.0f, 0.0f, 1.0f };
		else
			v = Vec4{ 0.0f, 0.0f, 1.0f, 1.0f };

		Vec4 background;
		memcpy(background.v, backgroundColor, sizeof(backgroundColor));
		std::vector<Vec4> pixels(canvasWidth * canvasHeight, background);
		pixels[(i % canvasWidth) + canvasWidth * (i / canvasWidth)] = v;
		//pixels[0 + canvasWidth * 0] = Vec4{ 1.0f, 1.0f, 0.0f, 1.0f };
		//pixels[2 + canvasWidth * 0] = Vec4{ 0.0f, 0.0f, 1.0f, 1.0f };
	...
	void SetBackgroundColor(float* v) {
		memcpy(backgroundColor, v, sizeof(backgroundColor));
	}
