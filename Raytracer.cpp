...
glm::vec3 Raytracer::RandomOffset( )
{
...
vec3 Raytracer::traceRay2x2(vec3 eyePos, vec3 pixelPos, const float dx, const int recursiveLevel)
{
	//:DEBUG:
...
vec3 Raytracer::traceRay(Ray& ray, const int recurseLevel)
{
	...
	if (hit.d >= 0.0f)
	{
		...
		if (m_enableShadows)
		{
			int numShadowSamples = 8; // 샘플링할 광원의 수
			const float maxDistance = 10.0f;

			if (false == m_enableSoftShadows) {
				numShadowSamples = 1;
			}
			for (int i = 0; i < numShadowSamples; i++)
			{
				...
				vec3 lightSamplePos = light.pos + RandomOffset( );
		...
		if (m_enableReflection && hit.obj->reflection)
		{
...
void Raytracer::UpdateLight(float dt)
{
...
vec3 Raytracer::TransformScreenToWorld(glm::vec2 posScreen)
{
